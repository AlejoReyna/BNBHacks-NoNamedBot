...
class BreakoutEngine:
    ...
    def _evaluate_cheap_candidate(
        ...
        # 5. RSI Factor (Optional)
        rsi_in_range = True
        if rsi is not None:
            rsi_in_range = 55.0 <= rsi <= 75.0

        # 6. Derivatives Factor (Optional)
    ...
    def _decision_from_candidate(
        ...
        # Slippage is fail-closed: missing/invalid slippage blocks entry even when other core factors pass.
    ...
    def _volume_breakout(
        ...
        """Prefer CMC 1h volume vs rolling 24h hourly average; fall back to local 24h cache."""
    ...
    def _breakout_high_break(
        ...
        """Prefer CMC high_3h when present; otherwise compare price to the local 3h cache."""
    ...
    def check_regime(self, token_data: dict[str, Any], bnb_data: dict[str, Any] | None = None) -> bool:
        """Return whether BNB and token trend filters are safe enough for entry."""
